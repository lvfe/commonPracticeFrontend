...
export class OverviewComponent implements AfterViewInit {
  private detailData = [];
  private detailStart = Date.UTC(2008, 7, 1);

  private detailChart = null;
  private dynamicChart = null;

  public detailHighCharts: typeof Highcharts = Highcharts;
  public detailchartOptions: Highcharts.Options = {
    chart: {
      marginBottom: 120,
      reflow: false,
      marginLeft: 50,
    ...
    series: [{
      type: 'line',
      name: '美元兑欧元',
      pointStart: this.detailStart,
      pointInterval: 24 * 3600 * 1000,
      data: this.detailData
    }]
  };
  public dyHighcharts: typeof Highcharts = Highcharts;
  public dychartOptions: Highcharts.Options = {
    chart: {
      reflow: false,
      borderWidth: 0,
      ...
      events: {
        ...
        selection: (event) => {
          var extremesObject = event.xAxis[0],
            ...
            detailData = [],
            xAxis = this.dynamicChart.xAxis[0];
          Highcharts.each(this.dynamicChart.series[0]['data'], function (d) {
            if (d.x > min && d.x < max) {
          ...
          this.detailChart.series[0].setData(detailData);
          return false;
        }
      }
    },
  ...
  ngAfterViewInit(): void {
    const self = this;
    Highcharts.each(this.dynamicChart.series[0]['data'], function (d) {
      if (d.x >= self.detailStart) {
        self.detailData.push(d.y);
      }
    });
    this.detailChart.series[0].setData(self.detailData);
  }

  chartInstanceDetail($event){
    this.detailChart = $event;
  }
  chartInstanceDynamic($event){
    this.dynamicChart =  $event;
  }
